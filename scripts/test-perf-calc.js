...
(async () => {
  ...
  const urthSeries = allPrices.filter(p => p.ticker === 'URTH');
  const otherPrices = allPrices.filter(p => p.ticker !== 'URTH');
  ...
  console.log(`Tx: ${tx.length}, Cf: ${cf.length}, Prices (non-URTH): ${otherPrices.length}, URTH: ${urthSeries.length}`);
  ...
  for (const d of result.dailySeries.slice(-5)) {
    console.log(`${d.date} nav=${d.nav.toFixed(2)} cash=${d.cash.toFixed(2)} mv=${d.market_value.toFixed(2)} twr_cum=${(d.twr_cum*100).toFixed(2)}% dd=${(d.drawdown*100).toFixed(2)}% urth=${d.urth_norm?.toFixed(4) || '—'}`);
  }
